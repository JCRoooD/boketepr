...
| 13 | Spanish copy + ally pass | done | Hazards render as Spanish phrases ("Tráfico alto"), not snake_case tokens — `lib/reports/hazard-labels.ts` maps canonical tokens to user-facing labels with accents, with a Spanish-title-case prettify fallback for unknown tokens. AI prompt (`HAZARD_VOCABULARY_HINT`) now suggests Spanish phrases instead of snake_case. Ally-pass guardrail at `lib/copy/invariants.test.ts` bans `bache`/`hueco` in user-facing source, asserts canonical severity labels + "hoyo" usage + AI prompt coverage. 25 node:test assertions across `hazard-labels.test.ts`, `invariants.test.ts`, `prompts.test.ts`. |
